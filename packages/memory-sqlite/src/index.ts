...
export function createSqliteMemoryProvider(
  ...
  return {
    ...
    async store(record) {
      ...
      opened.database
        .query("INSERT INTO lunar_memories (id, content, metadata, namespace, tenant_id, owner_id, expires_at, created_at) VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)")
        .run(stored.id, stored.content, stored.metadata === undefined ? null : JSON.stringify(stored.metadata), stored.namespace ?? null, stored.tenantId ?? null, stored.ownerId ?? null, stored.expiresAt ?? null, stored.createdAt);
      return stored;
    },
    async retrieve(query: MemoryQuery) {
      ...
      const rows = opened.database
        .query("SELECT id, content, metadata, namespace, tenant_id, owner_id, expires_at, created_at FROM lunar_memories WHERE content LIKE ?1 ESCAPE '\\' AND (?2 IS NULL OR namespace = ?2) AND (?3 IS NULL OR tenant_id = ?3) AND (?4 IS NULL OR owner_id = ?4) AND (expires_at IS NULL OR expires_at > ?5) ORDER BY rowid ASC")
        .all(`%${escapeLike(query.text)}%`, query.namespace ?? null, query.tenantId ?? null, query.ownerId ?? null, Date.now()) as Array<{
          id: string;
          content: string;
